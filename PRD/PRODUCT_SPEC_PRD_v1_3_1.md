# PRODUCT_SPEC_PRD v1.3.1 — 상업용 베딕 리포트 엔진 (Contract-Complete)

- **Status:** LOCKED (상세 계약 보존 + v1.3.1 실행 범위 분리)
...
- 상세 변경 이력: `CHANGELOG_PRD_v1_3_1.md`
- 현재 버전 핵심:
  - 현재 워크스페이스에서는 `python -m pytest`가 실행 가능하며, backend baseline은 `367 passed, 14 failed, 1 skipped`(`-p no:cacheprovider`) 상태임
...
- **LOCK 항목**은 v1.3.1 내에서 변경 금지입니다 (필요 시 후속 버전으로 버전업).
...
- v1.3.1의 유일한 P0 productization 대상은 `life_cycle-lite`입니다.
...
- `yearly_forecast`, `compatibility`는 스펙 유지 대상이지만 v1.3.1 출고 블로커는 아닙니다.
...
| `contract_version` | 문자열 | 적용 PRD 계약 버전 (`v1.3.1`) |
...
1. **인생 주기 리포트 (Life Cycle Map)**: 마하다샤 기반 인생 전체 주기 지도 + 현재 위치 + 방법론/valid_until UX + 단계별 확장 구조 *(v1.3.1 구현 단계: P0는 `life_cycle-lite`; 고점/저점 지도, 반복 패턴, 다음 3년 구체화는 P1에서 복원)*
2. **신년운세 리포트**: 연간 구간 + 월별 12슬롯 + 중요구간 2개 + 분야별 모듈
3. **궁합 리포트**: Ashtakoota 36점 기반 궁합 평가 + 조율 행동

> **v1.3.1 구현 단계 계약 (LOCK)**:
...
> - 아래 상세 계약은 보존되며, v1.3.1에서는 P1 기능 미구현만으로 FAIL 처리하지 않습니다.

#### 7.1.1 목적/핵심 가치

- **핵심 질문**: "내 인생의 큰 파도는 언제 오고, 언제 내려가는가?"
- **엔진 기반**: Vimshottari Dasha 120년 주기 전체 계산값 활용
- **구조 전략 (LOCK)**: `life_cycle-full` 목표 구조는 인생 전체 70% + 다음 3년 구체화 30%를 유지한다. 단, v1.3.1의 `life_cycle-lite` P0는 `7.1.2`의 최소 출력 구조만 적용하며, 다음 3년 구체화는 P1에서만 복원한다.
...
> **v1.3.1 P0 필수 섹션 집합 (exact H2, order LOCK)**:
> - `cover/meta`
> - `How to use 1p`
> - `인생 구조 한 장 요약`
> - `4단계 인생 구조`
...
#### 7.1.5 고점/저점 지도 산출 기준 (결정론, LOCK, v1.3.1 구현 단계: P1)
...
#### 7.1.6 반복 패턴 분석 기준 (결정론, LOCK, v1.3.1 구현 단계: P1)
...
#### 7.1.7 다음 3년 구체화 섹션 (재구매 훅, LOCK, v1.3.1 구현 단계: P1)
...
- **업셀 트리거 (결정론)**: *(v1.3.1 P0에서는 CTA-lite만 필수, 최적화는 P1)*
...
- v1.3.1 P0 수용 기준은 `life_cycle-lite` 최소 구조 중심으로 판정합니다.
- 7.1.5 / 7.1.6 / 7.1.7의 상세 테스트는 계약 보존 대상이지만, v1.3.1 P0 미구현만으로 FAIL 처리하지 않습니다.
...
- `CTA-lite` 하단에 동의 버튼/secondary control이 별도 존재 → `action_steps_contract_ok=False`  **(v1.3.1 P0)**
- generic front-contract/12챕터 scan 결과와 충돌하더라도, 최종 판정은 반드시 `life_cycle-lite` release mode 전용 helper 출력 기준으로 닫음

**인생 주기 리포트**:
- `assign_life_stages()`: 다샤 9개 → [2,2,2,3] / 4개 → [1,1,1,1] / 3개 → 3 stages / 1개 → 1 stage / 0개 → 빈 리스트  **(v1.3.1 P0 필수)**
...
- `compute_life_highs_lows()`: 9개 → 상위3/하위3/전환점5 / 5개 → 전환점 4개  **(v1.3.1 P1)**
...
- `compute_repeat_patterns()`: 동일 도메인 2회+ → 패턴 인식 / 1회 → 제외  **(v1.3.1 P1)**
...
- 다음 3년 구체화: 3개 슬롯 → 3개 출력 (패딩 없음) / 0개 → 고정 문구만 출력  **(v1.3.1 P1)**
...
> **운영 근거 (v1.3.1)**:
> - golden snapshot은 운영하지 않지만, v1.3.1는 P0 범위를 `life_cycle-lite`로 축소한 상태다.
...
> - 릴리즈 증적은 `PRD/release_evidence/v1_3_1/life_cycle_lite_manual_qa.md`, `PRD/release_evidence/v1_3_1/life_cycle_lite_sample_response.json`, `PRD/release_evidence/v1_3_1/life_cycle_lite_gate_summary.json`, `PRD/release_evidence/v1_3_1/life_cycle_lite_release_manifest.json` 4종으로 고정합니다.
...
> - `life_cycle_lite_sample_response.json`의 `meta.contract_version`, `life_cycle_lite_gate_summary.json`의 `contract_version`, `life_cycle_lite_release_manifest.json`의 `contract_version` / `release_evidence_dir` / `render_profile` / `request_fingerprint` / `evidence_case_id`는 모두 현재 정본 버전 `v1.3.1` 및 수동 QA 문서와 일치해야 합니다.
...
- **인생 주기 리포트 전용**:
  ...
  - v1.3.1 P0: 4단계 구조가 과하게 복잡하지 않고 한 번에 이해되는지 확인
  - v1.3.1 P0: P1 섹션 미노출이 오히려 문서를 더 명료하게 만드는지 확인
...
| **v1.3.1 (현재, 진행 중)** | **backend test baseline 재분류 + `life_cycle-lite` P0 productization + release evidence traceability hardening** |
| v1.2.x | `v1.3.1` 수용 기준 전부 충족 후 `life_cycle-full` 착수 / yearly_forecast·compatibility 정리 |
...
- 대응: v1.3.1 P0는 backend-only로 닫되, 전체 저장소 적용을 주장하려면 `frontend/app/page.tsx`, `frontend/app/chart/ChartClient.tsx`, `frontend/lib/api.ts`, 관련 E2E가 새 계약을 실제로 소비하도록 구현되어 있어야 합니다. migration 계획/노트는 보조 문서일 뿐 완료 근거가 아닙니다.
...
- 대응: v1.3.1에서는 `life_cycle-lite`까지만 P0로 닫고, full 기능은 P1로 분리.
...
16. `PRD/release_evidence/v1_3_1/` 아래에 `life_cycle_lite_manual_qa.md`, `life_cycle_lite_sample_response.json`, `life_cycle_lite_gate_summary.json`, `life_cycle_lite_release_manifest.json`가 모두 존재하고, 수동 QA 문서의 `contract_version`, `release_evidence_dir`, `render_profile`, `request_fingerprint`, `evidence_case_id`, `commit_sha`, `release_manifest_path`가 현재 정본 `v1.3.1` 기준과 일치함
