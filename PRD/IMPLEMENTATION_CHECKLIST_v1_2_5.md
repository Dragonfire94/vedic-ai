# IMPLEMENTATION_CHECKLIST v1.2.5 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_4.md` + `PATCH_v1_2_4_to_v1_2_5.md`
- 점검 기준일: 2026-03-06
...
| `commercial_gate_helpers.py` | `TODO` | 파일 자체 없음 |
| `product_orchestrator.py` | `TODO` | 파일 자체 없음 |
| `render_contract.py` | `TODO` | 파일 자체 없음 |
| `pytest` 실행 환경 | `BLOCKED` | 현재 설치 안 됨 |
| micro fixture 체계 | `TODO` | generic fixture 1개만 있음 |
| token budget v1.2.5 정렬 | `PARTIAL` | 코드 상수와 PRD 수치 불일치 |
...
14. `TODO` `render_contract.py` 부재
    ...
    - 해야 할 일:
      - 응답 메타 계약 (`product_type`, `contract_version`, `render_profile`, `valid_until`, fallback flags) 정의
...
26. `BLOCKED` 현재 테스트 스위트 일부가 코드와 불일치할 가능성 높음
    ...
    - 해야 할 일:
      ...
      - v1.2.5용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식이 더 안전한지 결정
...
29. `TODO` `cheap_validation_gate`에 life_cycle-lite 모드 없음
    - 현재 상태: generic front-contract/12챕터 기준
    - 해야 할 일:
      - 새 상품용 lightweight scan 모드 추가 또는 별도 validator 추가
...
3. `backend/commercial_gate_helpers.py`
   - 역할: pure function 모음
   - 포함: stage grouping, planet label map, valid_until lifecycle

4. `backend/life_cycle_lite_renderer.py`
...
PRD v1.2.5 요구:
- `life_cycle-lite` 기본 `7000`
- 권장 `6000–8000`
- 예외 상한 `9000`

권장 구현:
1. global hard limit는 당장 깨지 말 것
2. `product_type == life_cycle` 경로에서만 product-layer 기본값 `7000` 적용
3. `life_cycle` 경로에서만 soft/hard validation `6000–8000 / 9000` 적용
4. generic `/ai_reading`와 충돌하지 않게 product contract에서 통제
...
8. `TODO` `/ai_reading` 확장 또는 thin wrapper 결정
   - 권장: 기존 `/ai_reading` 유지 + optional `product_type` 추가
...
11. `TODO` `commercial_gate_helpers.py` 신설
    - P0 필수 상수:
      - `PLANET_LABEL_MAP`
      - `LIFE_STAGE_LABELS`
    - P0 필수 함수:
...
21. `TODO` 방법론 카드 6줄 deterministic copy 구현
    - 현재 generic front를 그대로 쓸지, life_cycle 전용 card를 둘지 결정 필요
    - 권장: product renderer 내부 고정 문구로 먼저 닫기
    - 결정 시점: Phase B 종료 전 확정
    - 차단 규칙: 아이템 21 결정 전에는 아이템 26~30 착수 금지 (HF15/방법론 카드 기준을 먼저 잠가야 함)
...
26. `TODO` life_cycle-lite contract validator 추가
    - 선택안 A: `cheap_validation_gate.py`에 product mode 추가
    - 선택안 B: `backend/test_life_cycle_lite_contract.py` + pure checks
    - 권장: 처음엔 B가 더 가볍다
    - 통합 계획: P0는 B로 시작하되, P0 완료 직후 A로 흡수하는 것을 기본 계획으로 둔다

27. `TODO` 최소 contract test 1: 메타 필드
    - 체크:
      - `product_type == life_cycle`
      - `contract_version == v1.2.5` 또는 구현 당시 최신값
...
33. `TODO` cache key에 `product_type` 반영
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_5.md` 정본 생성 시 코드와 맞춰 재검수
...
아래가 모두 만족되면 v1.2.5 기준 P0 완료로 봅니다.
...
8. 단일 LLM 호출 정책 유지
9. `life_cycle` 상품 경로의 기본 token 정책이 `7000 / 6000–8000 / 9000`으로 고정됨
