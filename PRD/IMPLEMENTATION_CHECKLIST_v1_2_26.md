# IMPLEMENTATION_CHECKLIST v1.2.26 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_26.md`
- 점검 기준일: 2026-03-10
...
- Target state: v1.2.26 P0는 backend-only `life_cycle-lite` productization 계약을 잠그는 문서이며, repo-wide 완료 주장은 README/frontend migration까지 반영된 뒤에만 가능합니다. 단, backend-only라도 product-aware finalizer/cache/PDF contract는 함께 닫혀 있어야 합니다.

---

## 0.6 필수 리스크 6개 (2026-03-10 latest)
...
| token budget v1.2.26 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
`AUD-25` `BLOCKED` `python -m pytest` clean-env / execution contract broken (release blocker)
    ...
    - release 기준: 이 항목이 닫히기 전에는 v1.2.26 P0 출고를 닫지 않음
    - 해야 할 일:
...
`AUD-26` `BLOCKED` stale test 정렬 필요: `backend/test_llm_token_limits.py`
    ...
    - 해야 할 일:
      ...
      - v1.2.26용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.26에서 직접 변경 대상이 아님

PRD v1.2.26 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_26.md` 정본과 코드 정합 재검수
...
10. contract test 4종 + `cheap_validation_gate` release mode 추가 (`Phase E` #25~#32)
11. `backend/QUALITY_GATES.md` / `README.md` release gate source 전환 (`Phase G` #38~#41)
12. fallback/edge 수동 QA 2건 수행 (`Phase E` #30~#31)
13. repo-wide 적용이 필요하면 frontend/client note만이 아니라 `frontend/app/page.tsx`, `frontend/lib/api.ts`, `frontend/app/chart/ChartClient.tsx` 구현까지 반영 (`Phase G` #42)
14. 그 다음에만 개선 작업
...
아래가 모두 만족되면 v1.2.26 기준 P0 완료로 봅니다.
...
16. `backend/API.md`가 `/ai_reading`의 `product_type`, `life_cycle` 요청 입력, 응답 meta contract를 현재 기준으로 설명함
