# PRODUCT_SPEC_PRD v1.2.10 — 상업용 베딕 리포트 엔진 (Contract-Complete)

- **Status:** LOCKED (상세 계약 보존 + v1.2.10 실행 범위 분리)
...
- **LOCK 항목**은 v1.2.10 내에서 변경 금지입니다 (필요 시 후속 버전으로 버전업).
...
- v1.2.10의 유일한 P0 productization 대상은 `life_cycle-lite`입니다.
- `yearly_forecast`, `compatibility`는 스펙 유지 대상이지만 v1.2.10 출고 블로커는 아닙니다.
...
| `contract_version` | 문자열 | 적용 PRD 계약 버전 (`v1.2.10`) |
...
1. **인생 주기 리포트 (Life Cycle Map)**: 마하다샤 기반 인생 전체 주기 지도 + 현재 위치 + 방법론/valid_until UX + 단계별 확장 구조 *(v1.2.10 구현 단계: P0는 `life_cycle-lite`; 고점/저점 지도, 반복 패턴, 다음 3년 구체화는 P1에서 복원)*
2. **신년운세 리포트**: 연간 구간 + 월별 12슬롯 + 중요구간 2개 + 분야별 모듈
3. **궁합 리포트**: Ashtakoota 36점 기반 궁합 평가 + 조율 행동

> **v1.2.10 구현 단계 계약 (LOCK)**:
...
> - 아래 상세 계약은 보존되며, v1.2.10에서는 P1 기능 미구현만으로 FAIL 처리하지 않습니다.
...
> **v1.2.10 P0 필수 섹션 집합 (exact H2, order LOCK)**:
> - `cover/meta`
> - `How to use 1p`
> - `인생 구조 한 장 요약`
> - `4단계 인생 구조`
...
#### 7.1.5 고점/저점 지도 산출 기준 (결정론, LOCK, v1.2.10 구현 단계: P1)
...
#### 7.1.6 반복 패턴 분석 기준 (결정론, LOCK, v1.2.10 구현 단계: P1)
...
#### 7.1.7 다음 3년 구체화 섹션 (재구매 훅, LOCK, v1.2.10 구현 단계: P1)
...
- **업셀 트리거 (결정론)**: *(v1.2.10 P0에서는 CTA-lite만 필수, 최적화는 P1)*
...
### 10.0 v1.2.10 구현 단계 계약 (LOCK)

- 표준 테스트 실행 명령은 `python -m pytest`입니다.
- `pytest` CLI PATH 의존은 허용하지 않습니다.
- v1.2.10 P0 수용 기준은 `life_cycle-lite` 최소 구조 중심으로 판정합니다.
- 7.1.5 / 7.1.6 / 7.1.7의 상세 테스트는 계약 보존 대상이지만, v1.2.10 P0 미구현만으로 FAIL 처리하지 않습니다.
...
**인생 주기 리포트**:
- `assign_life_stages()`: 다샤 9개 → [2,2,2,3] / 4개 → [1,1,1,1] / 3개 → 3 stages / 1개 → 1 stage / 0개 → 빈 리스트  **(v1.2.10 P0 필수)**
...
- `compute_life_highs_lows()`: 9개 → 상위3/하위3/전환점5 / 5개 → 전환점 4개  **(v1.2.10 P1)**
...
- `compute_repeat_patterns()`: 동일 도메인 2회+ → 패턴 인식 / 1회 → 제외  **(v1.2.10 P1)**
...
- 다음 3년 구체화: 3개 슬롯 → 3개 출력 (패딩 없음) / 0개 → 고정 문구만 출력  **(v1.2.10 P1)**
...
> **운영 근거 (v1.2.10)**:
> - golden snapshot은 운영하지 않지만, v1.2.10은 P0 범위를 `life_cycle-lite`로 축소한 상태다.
...
- CTA 앞 섹션과 의미 연결 확인
- 온보딩 목표가 인생 구조 한 장 요약/모듈 순서에 반영됐는지 육안 확인
...
- **인생 주기 리포트 전용**:
  ...
  - v1.2.10 P0: 4단계 구조가 과하게 복잡하지 않고 한 번에 이해되는지 확인
  - v1.2.10 P0: P1 섹션 미노출이 오히려 문서를 더 명료하게 만드는지 확인
...
| **v1.2.10 (현재)** | **HF11/HF12 product scope 정렬 / summary H2 exact-name 통일 / CTA-lite adjacency 잠금** |
| v1.2.x | `v1.2.10` 수용 기준 전부 충족 후 `life_cycle-full` 착수 / yearly_forecast·compatibility 정리 |
...
- 대응: v1.2.10에서는 `life_cycle-lite`까지만 P0로 닫고, full 기능은 P1로 분리.
