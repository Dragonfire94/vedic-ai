# IMPLEMENTATION_CHECKLIST v1.2.31 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_31.md`
- 점검 기준일: 2026-03-10
...
- Current state: 현재 워크스페이스 인터프리터에서는 `python -m pytest` 자체가 설치되지 않아 (`C:\Python314\python.exe: No module named pytest`) acceptance command를 실행 재현할 수 없습니다.
...
- Target state: v1.2.31 P0는 backend-only `life_cycle-lite` productization 계약을 잠그는 문서이며, repo-wide 완료 주장은 README/frontend migration까지 반영된 뒤에만 가능합니다. 단, backend-only라도 product-aware finalizer/cache/PDF contract는 함께 닫혀 있어야 합니다.

---

## 0.6 필수 리스크 7개 (2026-03-10 latest)

1. 현재 워크스페이스에서는 `python -m pytest --version`부터 `No module named pytest`로 실패하며, `backend/requirements.txt`에는 `pytest`가 없고 `backend/test_llm_token_limits.py`도 현재 런타임과 충돌합니다.
...
| `pytest` 실행 환경 | `BLOCKED` | 현재 워크스페이스 인터프리터에는 `pytest`가 없어 표준 명령 자체가 실행되지 않음 |
...
| token budget v1.2.31 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
`AUD-25` `BLOCKED` `python -m pytest` clean-env / execution contract broken (release blocker)
    - 근거: 현재 환경에서는 `python -m pytest --version`이 실패 (`No module named pytest`)
    - 추가 근거: `backend/requirements.txt`에 `pytest` 없음
    - 추가 근거: 루트에 `pytest.ini`, `pyproject.toml`, `tox.ini`가 없어 수집 제외 규칙도 고정돼 있지 않음
    - 현재 재현: `python -m pytest --version`, `python -m pytest backend/test_llm_token_limits.py -q` 모두 실행 단계에서 실패
    - 의미: 현재 워크스페이스에서는 collection 문제를 보기 전에 표준 테스트 명령 자체가 재현되지 않음
    - release 기준: 이 항목이 닫히기 전에는 v1.2.31 P0 출고를 닫지 않음
    - 해야 할 일:
      - `pytest` 설치 경로부터 문서화 (`requirements-dev` 또는 동등 경로)
      - 그 다음 필요 시 pytest 수집 제외 규칙 고정 (`logs/`, temp/cache 계열)
...
`AUD-26` `BLOCKED` stale test 정렬 필요: `backend/test_llm_token_limits.py`
    ...
    - 현재 상태: `pytest`가 설치되지 않은 현재 워크스페이스에서는 실행 재현 전 단계에서 막히지만, 코드 기준 stale expectation은 여전히 정리 대상임
    - 해야 할 일:
      - 이 테스트를 현재 아키텍처에 맞게 정리
      - v1.2.31용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.31에서 직접 변경 대상이 아님

PRD v1.2.31 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
1. `BLOCKED` `python -m pytest` clean-env 재현성 확보
   - 현재 상태: 현재 인터프리터에서는 `python -m pytest --version` 실패 (`No module named pytest`)
...
25. `BLOCKED` `python -m pytest` clean-env / execution contract broken (release blocker)
...
31. `TODO` 수동 QA 템플릿 작성
    - 권장 경로: `PRD/release_evidence/v1_2_31/life_cycle_lite_manual_qa.md`
    ...
    - 필수 필드:
      ...
      - reviewer
      - run date (Asia/Seoul)
    - Done 기준: 2건 모두 채워져 있고, 문서 상단 메타만 읽어도 현재 정본 `v1.2.31` 기준의 release candidate와 연결된 manifest 경로를 확인할 수 있음

31-a. `TODO` release gate summary 저장
    - 권장 경로: `PRD/release_evidence/v1_2_31/life_cycle_lite_gate_summary.json`
    ...
    - Done 기준: 자동 gate가 실제 release source of truth로 통과했는지 별도 로그 검색 없이 확인 가능하고, `contract_version == v1.2.31`이며 `render_profile` / `request_fingerprint` / `evidence_case_id`가 수동 QA 문서 및 manifest와 일치함

31-b. `TODO` 샘플 응답 1개 저장
    - 권장 경로: `PRD/release_evidence/v1_2_31/life_cycle_lite_sample_response.json`
    ...
    - Done 기준: 문서/QA/release gate 검토자가 코드 실행 없이도 계약 필드와 최종 렌더 구조를 확인할 수 있고, `meta.contract_version == v1.2.31`이며 `meta.render_profile`이 gate summary 및 manifest와 일치함

31-c. `TODO` release evidence manifest 저장
    - 권장 경로: `PRD/release_evidence/v1_2_31/life_cycle_lite_release_manifest.json`
    ...
    - 필수 포함:
      ...
      - `gate_summary_sha256`
    - Done 기준: reviewer가 manifest 하나만 읽어도 세 증적 파일의 경로/해시/버전/render/request identity를 현재 정본 `v1.2.31` 기준으로 재검수할 수 있음
...
39. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_31.md` 정본과 코드 정합 재검수
...
6. `commercial_quality_constants.py` shared constants + `life_cycle_helpers.py` 추가 (`Phase C` #11~#20)
7. `life_cycle-lite` payload builder 구현 (`Phase C` #12~#17)
8. `life_cycle_lite_renderer.py` 구현 (`Phase D` #19~#24)
9. response meta local-date 직렬화 + `/pdf` direct call contract 정렬 (`Phase F` #34~#36)
...
12. 수동 QA 2건 + `release_evidence` 4종(`manual_qa`, `sample_response`, `gate_summary`, `release_manifest`) 저장 (`Phase E` `#30~#31-c`)
...
아래가 모두 만족되면 v1.2.31 기준 P0 완료로 봅니다.
...
19. `PRD/release_evidence/v1_2_31/` 아래에 `life_cycle_lite_manual_qa.md`, `life_cycle_lite_sample_response.json`, `life_cycle_lite_gate_summary.json`, `life_cycle_lite_release_manifest.json`가 모두 존재하고, manifest를 포함한 4종이 같은 `contract_version`, `render_profile`, `request_fingerprint`, `evidence_case_id`, `commit_sha` 기준으로 현재 정본 `v1.2.31`과 일치함
