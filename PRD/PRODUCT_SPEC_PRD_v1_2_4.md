...
| null (입력 없음) | `as_of_local + 3년` + `fallback_applied=True` | **소비자 UX 규칙 적용** |
| 파싱 실패 | 파이프라인에서 `None` 변환 → null 케이스 동일 | **소비자 UX 규칙 적용** |
| `next_mahadasha_date < as_of_local` | `as_of_local + 3년` + 운영 알림 + `fallback_applied=True` | **소비자 UX 규칙 적용** |

**소비자 UX 규칙 — fallback_applied=True 시 (LOCK)**:
...
| `commercial_gate_helpers.py` | 알고리즘 함수 | 패턴 상수 재정의 |
...
- `valid_until_fallback`: fallback_applied=True 시 기록
