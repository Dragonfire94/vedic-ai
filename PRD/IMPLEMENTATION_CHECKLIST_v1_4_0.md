# IMPLEMENTATION_CHECKLIST v1.4.0 (Single Product Promotion Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_4_0.md`
- 점검 기준일: 2026-03-11
- 점검 범위: 현재 저장소의 backend 중심 구현 상태, baseline 안정화, target report 승격, API 표면, 후처리/게이트, 문서-코드 불일치
- 목적: "하나의 베딕 라이프사이클 리포트를 현재 baseline에서 목표 보고서까지 어떻게 끌어올릴지"를 코드 기준으로 잠그는 실행 체크리스트
...
- `P1`: baseline cut 범위 밖이며, v1.4.0의 target report phase에서 다룸
...
현재 프로젝트는 "별도 lite 상품과 full 상품을 새로 따로 만들어야 하는 상태"가 아니라, 아래 조합이 이미 있는 상태입니다.

- 엔진 코어 있음
- generic AI reading 파이프라인 있음
- polished surface / cache / cheap gate 있음
- 다샤 timing 보조 컨텍스트 있음
- baseline path로 정리할 수 있는 life_cycle 계약 초안이 이미 문서화돼 있음

문제는 **단일 상품 오케스트레이션과 단계 승격 계층**이 없다는 것입니다.

즉, 이번 구현의 본질은 엔진 개발이 아니라 아래 8개입니다.

1. baseline path를 단일 상품 기준으로 고정
2. 상품 오케스트레이션 추가
3. baseline deterministic payload/renderer 추가
4. product-aware finalizer / front 격리
5. 메타 + cache + PDF 계약 고정
6. baseline test / gate / release evidence 정리
7. target report용 고점/저점 / 반복 패턴 / 다음 3년 승격
8. editorial QA와 single-product cutover 기준 확정

이 8개를 순서대로 닫으면, 외부 상품은 하나로 유지하면서도 현재 baseline에서 목표 보고서까지 무리 없이 올라갈 수 있습니다.
...
- 이 값은 v1.4.0에서 직접 변경 대상이 아님

PRD v1.4.0 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
18. `TODO` baseline cut 전까지 target section을 released path에 섞지 않기
    - 대상: 전환 강도/반복 패턴/다음 3년 구체화
    - Done 기준: baseline gate가 닫히기 전에는 target section이 release path에 노출되지 않고, target cutover phase에서만 승격됨
...
## Phase D-a. target report 승격

24-a. `TODO` `7.1.5` 고점/저점 지도 deterministic payload 구현
    - Done 기준: 상승 3 / 경계 3 / 전환점 5(또는 edge rule)가 pure helper 기준으로 재현됨

24-b. `TODO` `7.1.6` 반복 패턴 분석 deterministic payload 구현
    - Done 기준: 동일 도메인 2회+ 패턴만 노출되고, 1회성 잡음은 제외됨

24-c. `TODO` `7.1.7` 다음 3년 구체화 deterministic payload 구현
    - Done 기준: 0개면 고정 문구, N개면 N개만 출력하며 패딩 없음

24-d. `TODO` target report renderer / assembly branch 추가
    - 권장 파일: `backend/life_cycle_target_renderer.py`
    - Done 기준: 현재 baseline markdown을 억지로 덧대는 방식이 아니라, 같은 보고서의 target-stage section order가 dedicated branch로 존재

24-e. `TODO` target report exact H2 order와 section insertion contract 고정
    - 최소 요구: baseline H2를 유지하면서 `인생 고점/저점 지도`, `반복 패턴 분석`, `다음 3년 구체화`가 canonical 위치에 각 1회 삽입
    - Done 기준: target report H2 order가 deterministic하며 alias/중복이 없음

24-f. `TODO` target-stage personalization deepening
    - 입력 반영: `onboarding_goal`, `occupation_context`, `relationship_status`, `concern_tokens`
    - Done 기준: 요약 / 현재 위치 / 다음 3년 중 최소 2개 이상에서 개인화 흔적이 명확함

24-g. `TODO` target report contract / gate test 추가
    - 권장 파일: `backend/test_life_cycle_target_contract.py`
    - Done 기준: baseline path와 target path 모두 현재 PRD 기준 exact contract test로 구분 검증 가능

24-h. `TODO` editorial QA rubric + sample 20건 리뷰
    - Done 기준: "술술 읽힘" / "내 얘기처럼 읽힘" / "행동 연결" / "과한 jargon 억제" 기준으로 20건 리뷰 기록 존재
...
31. `TODO` 수동 QA 템플릿 작성
    - 권장 경로: `PRD/release_evidence/v1_4_0/life_cycle_lite_manual_qa.md`
    ...
    - 필수 필드:
      ...
      - reviewer
      - run date (Asia/Seoul)
    - Done 기준: 2건 모두 채워져 있고, 문서 상단 메타만 읽어도 현재 정본 `v1.4.0` 기준의 release candidate와 연결된 manifest 경로를 확인할 수 있음

31-a. `TODO` release gate summary 저장
    - 권장 경로: `PRD/release_evidence/v1_4_0/life_cycle_lite_gate_summary.json`
    ...
    - Done 기준: 자동 gate가 실제 release source of truth로 통과했는지 별도 로그 검색 없이 확인 가능하고, `contract_version == v1.4.0`이며 `render_profile` / `request_fingerprint` / `evidence_case_id`가 수동 QA 문서 및 manifest와 일치함

31-b. `TODO` 샘플 응답 1개 저장
    - 권장 경로: `PRD/release_evidence/v1_4_0/life_cycle_lite_sample_response.json`
    ...
    - Done 기준: 문서/QA/release gate 검토자가 코드 실행 없이도 계약 필드와 최종 렌더 구조를 확인할 수 있고, `meta.contract_version == v1.4.0`이며 `meta.render_profile`이 gate summary 및 manifest와 일치함

31-c. `TODO` release evidence manifest 저장
    - 권장 경로: `PRD/release_evidence/v1_4_0/life_cycle_lite_release_manifest.json`
    ...
    - 필수 포함:
      ...
      - `gate_summary_sha256`
    - Done 기준: reviewer가 manifest 하나만 읽어도 세 증적 파일의 경로/해시/버전/render/request identity를 현재 정본 `v1.4.0` 기준으로 재검수할 수 있음


31-d. `TODO` target report evidence 4종 추가
    - 권장 경로:
    ...
    - Done 기준: reviewer가 target manifest 하나만 읽어도 target report 증적 3종의 경로/해시/버전/render/request identity를 현재 정본 `v1.4.0` 기준으로 재검수할 수 있음
...
1. baseline gate를 닫기 전에 target section(고점/저점 지도, 반복 패턴, 다음 3년 구체화)을 released path에 섞는 것
...
## 7. 추천 구현 순서 (현실적인 single-product 순서)
...
6. `life_cycle` pure helper / payload builder / baseline renderer 정리 (`Phase C` + `Phase D`)
7. response meta local-date 직렬화 + `/pdf` direct call contract 정렬 (`Phase F` #34~#36)
8. `cheap_validation_gate` baseline release mode + micro fixture / manual QA / release evidence 4종 정리 (`Phase E`)
9. `backend/API.md` + `backend/QUALITY_GATES.md` + `README.md` 문서 계약 동기화 (`Phase G` #37~#40)
10. target report용 고점/저점 / 반복 패턴 / 다음 3년 / target renderer / target gate / editorial QA 20건 순으로 승격 (`Phase D-a` + `Phase E`)
11. repo-wide 적용이 필요하면 frontend consumer 구현까지 반영 (`Phase G` #41)

---

## 8. 완료 정의

### 8.1 Baseline cut 완료 정의

아래가 모두 만족되면 v1.4.0 기준 baseline cut 완료로 봅니다.
...
5. 응답에 baseline meta contract(`as_of_utc`, `as_of_local`, `timezone_offset`, `valid_until`, `valid_until_fallback`, `onboarding_goal`, `current_mahadasha_planet`, `next_mahadasha_date`, `product_type`, `contract_version`, `render_profile`) 존재
6. baseline markdown이 `cover/meta` 포함 exact H2 order로 deterministic 하게 생성되며, generic front modules가 finalize 단계에서 다시 붙지 않음
...
15. target section이 release path에 섞이지 않음
16. `backend/API.md`가 `/ai_reading`의 `product_type`, `life_cycle` 요청 입력, 응답 meta contract, backend-only baseline 한계를 현재 기준으로 설명함
17. README/운영 문서가 backend baseline과 repo-wide rollout을 구분함
18. micro fixture 2건 + 수동 QA 2건 통과
19. `PRD/release_evidence/v1_4_0/` 아래에 `life_cycle_lite_manual_qa.md`, `life_cycle_lite_sample_response.json`, `life_cycle_lite_gate_summary.json`, `life_cycle_lite_release_manifest.json`가 모두 존재하고, manifest를 포함한 4종이 같은 `contract_version`, `render_profile`, `request_fingerprint`, `evidence_case_id`, `commit_sha` 기준으로 현재 정본 `v1.4.0`과 일치함

> 주의: 위 완료 정의는 backend baseline 기준입니다. 프로젝트 전체 적용 완료를 주장하려면 frontend 진입 경로, `/ai_reading` client contract, chart consumer, E2E가 새 계약을 실제로 소비해야 하며, migration note/계획만으로는 충분하지 않습니다.

### 8.2 Target report cut 완료 정의

아래가 모두 만족되면 v1.4.0 기준 target report cut 완료로 봅니다.

1. Baseline cut 1-19를 모두 유지함
2. `7.1.5` 고점/저점 지도 deterministic payload가 구현되고 pure contract test를 통과함
3. `7.1.6` 반복 패턴 분석 deterministic payload가 구현되고 1회성 잡음이 제외됨
4. `7.1.7` 다음 3년 구체화가 패딩 없이 deterministic 하게 구현됨
5. target report가 하나의 보고서 안에서 canonical H2 order로 렌더되며, 외부 노출 문구에 `lite/full` SKU 표현이 없음
6. target report gate 또는 동등 전용 runner 기준 HF 16개 회귀 없음
7. 요약 / 현재 위치 / 다음 3년 중 최소 2개 이상에서 개인화 흔적이 명확함
8. editorial QA rubric 기반 sample 20건 리뷰 기록이 존재하고, 읽힘 / 납득 / 행동 연결 / 과한 jargon 억제 기준을 통과함
9. `PRD/release_evidence/v1_4_0/` 아래에 `life_cycle_target_manual_qa.md`, `life_cycle_target_sample_response.json`, `life_cycle_target_gate_summary.json`, `life_cycle_target_release_manifest.json`가 존재하고 reviewer entrypoint로 재검수 가능함
10. `backend/API.md` / `backend/QUALITY_GATES.md` / `README.md`가 single-product wording과 현재 release gate source of truth를 설명함
11. repo-wide rollout을 주장하려면 frontend 진입 경로, `/ai_reading` client contract, chart consumer, E2E가 새 계약을 실제로 소비함
