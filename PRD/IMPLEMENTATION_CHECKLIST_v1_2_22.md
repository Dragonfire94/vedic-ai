# IMPLEMENTATION_CHECKLIST v1.2.22 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_22.md`
- 점검 기준일: 2026-03-06
...
| frontend/client migration note | `PARTIAL` | 체크리스트에는 note 추가됨, README/API/frontend 반영은 아직 미완료 |
| micro fixture 체계 | `TODO` | generic fixture 1개만 있음 |
| token budget v1.2.22 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
22. `TODO` full P0 meta contract가 실제 backend 응답에 없음
    - 근거: backend 전체 검색 기준 `as_of_local`, `onboarding_goal`, `next_mahadasha_date`, `contract_version`, `render_profile`, `product_type` 응답 매치 없음
    - 해야 할 일:
...
25. `BLOCKED` `python -m pytest` dependency contract stale (release blocker)
    - 근거: 현재 환경에서는 `python -m pytest --version`이 성공 (`pytest 9.0.2`)
    - 추가 근거: `backend/requirements.txt`에 `pytest` 없음
    - 의미: 현재 로컬 인터프리터에 우연히 설치돼 있을 뿐, clean environment 재현성은 보장되지 않음
    - release 기준: 이 항목이 닫히기 전에는 v1.2.22 P0 출고를 닫지 않음
    - 해야 할 일:
...
26. `BLOCKED` stale test 정렬 필요: `backend/test_llm_token_limits.py`
    ...
    - 해야 할 일:
      ...
      - v1.2.22용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.22에서 직접 변경 대상이 아님

PRD v1.2.22 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
37. `PARTIAL` `backend/API.md`에 `/ai_reading` `product_type` / `life_cycle` 공개 계약 추가
    ...
    - 현재 상태: 공개 API 계약 골격은 문서화됨, 남은 쟁점은 GET list 직렬화 규칙과 구현 반영 정합성
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_22.md` 정본과 코드 정합 재검수
...
아래가 모두 만족되면 v1.2.22 기준 P0 완료로 봅니다.
