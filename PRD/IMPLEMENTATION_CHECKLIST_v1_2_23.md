# IMPLEMENTATION_CHECKLIST v1.2.23 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_23.md`
- 점검 기준일: 2026-03-06
...
- `P1`: v1.2.23 범위 밖이므로 지금 구현하지 않음

---

## 0.5 Current State vs Target State

- Current state: runtime backend는 아직 generic `GET /ai_reading` 단일 경로이며 `product_type` / `life_cycle-lite` 전용 분기가 없습니다.
...
- Target state: v1.2.23 P0는 backend-only `life_cycle-lite` productization 계약을 잠그는 문서이며, repo-wide 완료 주장은 README/frontend migration까지 반영된 뒤에만 가능합니다.
...
| `pytest` 실행 환경 | `BLOCKED` | clean env 재현성과 stale test 정렬 전까지 release blocker |
| frontend/client migration note | `PARTIAL` | 체크리스트와 API.md에는 반영됨, README/frontend 반영은 아직 미완료 |
| micro fixture 체계 | `TODO` | generic fixture 1개만 있음 |
| token budget v1.2.23 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
25. `BLOCKED` `python -m pytest` clean-env / collection contract broken (release blocker)
    ...
    - 현재 재현: `python -m pytest -q` 기준 `logs/tmp*`, `pytest-cache-files-*`, `tmp*` 수집 단계에서 `149`개 `PermissionError`로 중단
    - 의미: 현재 로컬 인터프리터에 우연히 설치돼 있을 뿐 아니라, 현 워크스페이스에서는 기본 수집 계약 자체도 깨져 있음
    - release 기준: 이 항목이 닫히기 전에는 v1.2.23 P0 출고를 닫지 않음
    - 해야 할 일:
      - pytest 수집 제외 규칙부터 고정 (`logs/`, temp/cache 계열)
      - dev requirements 또는 requirements 분리
      - 최소한 로컬/CI에서 문서화된 명령 1개로 `python -m pytest` 가능하게 만들 것26. `BLOCKED` stale test 정렬 필요: `backend/test_llm_token_limits.py`
    ...
    - 해야 할 일:
      ...
      - v1.2.23용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.23에서 직접 변경 대상이 아님

PRD v1.2.23 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
> - frontend/client migration은 backend P0 blocker는 아니지만, repo-wide 적용 완료를 주장하기 전에는 문서 또는 구현이 필요하다.
...
25. `BLOCKED` `python -m pytest` clean-env / collection contract broken (release blocker)
    ...
    - 해야 할 일:
      - pytest 수집 제외 규칙부터 고정 (`logs/`, temp/cache 계열)
      - dev requirements 또는 requirements 분리
      - 최소한 로컬/CI에서 문서화된 명령 1개로 `python -m pytest` 가능하게 만들 것26. `TODO` life_cycle-lite micro fixture contract test 추가
...
37. `PARTIAL` `backend/API.md`에 `/ai_reading` `product_type` / `life_cycle` 공개 계약 추가
    ...
    - 현재 상태: 공개 API 계약과 GET list CSV 직렬화 규칙까지 문서화됨, 남은 쟁점은 구현 반영 정합성
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_23.md` 정본과 코드 정합 재검수
41. `TODO` `README.md` 범위/출고 기준 정리
    ...
    - repo-wide 적용 완료가 아니라면 frontend/client migration 후속 작업이 남아 있음을 명시
42. `TODO` frontend/client/API/cache migration note 추가 (repo-wide 후속)
    - 대상 후보: `frontend/app/page.tsx`, `frontend/app/chart/ChartClient.tsx`, `frontend/lib/api.ts`
    - 의미: backend P0 blocker는 아니지만 전체 저장소 적용 문서로 쓰려면 필요
...
12. repo-wide 적용이 필요하면 frontend/client migration note 또는 구현 추가
13. PDF 경로 점검
14. 그 다음에만 개선 작업
...
아래가 모두 만족되면 v1.2.23 기준 P0 완료로 봅니다.
