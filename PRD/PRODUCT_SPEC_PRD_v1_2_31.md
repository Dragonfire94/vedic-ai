# PRODUCT_SPEC_PRD v1.2.31 — 상업용 베딕 리포트 엔진 (Contract-Complete)

- **Status:** LOCKED (상세 계약 보존 + v1.2.31 실행 범위 분리)
- **Last updated (Asia/Seoul):** 2026-03-10
- **Scope focus:**
  - 안정적인 상업 리포트 출력
  - `life_cycle-lite` backend P0 productization
  - HF gate / contract / micro fixture alignment
...
- 상세 변경 이력: `CHANGELOG_PRD_v1_2_31.md`
- 현재 버전 핵심:
  - 현재 워크스페이스에서는 `python -m pytest`가 설치되지 않아 (`No module named pytest`) 테스트 실행 계약이 먼저 복구돼야 함
  - 런타임 `/ai_reading` API / route cache key 미정렬뿐 아니라 generic finalizer/front 재부착 위험을 반영
  - polished narrative cache namespace와 `/pdf` direct-call contract도 product-aware 정책으로 함께 잠가야 함을 명시
  - frontend consumer / E2E 미반영 시 repo-wide 완료 주장 금지 명시
  - release gate source of truth 충돌을 선행 블로커로 고정
  - `Current State vs Target State` 감사 섹션은 중복을 피하기 위해 체크리스트 `0.5`로 이동
  - `7.1.1`의 70/30 구조 표현은 `life_cycle-full` 목표 구조로만 유지하고, `life_cycle-lite` P0 최소 구조와 충돌하지 않도록 정렬
  - `4.1 공통 출력 구조`에는 `life_cycle-lite` P0 exact H2 order 우선 규칙과 `CTA-lite -> 면책/윤리/데이터 보호` 순서를 명시
  - `contract test 4종`, `backend/API.md`, `backend/QUALITY_GATES.md`, `README.md` 종료 조건을 같은 수용 기준으로 정렬
  - `release_evidence`는 `manual_qa`, `sample_response`, `gate_summary`, `release_manifest` 4종으로 고정
  - 릴리즈 증적 4종은 같은 `contract_version`, `render_profile`, `request_fingerprint`, `evidence_case_id`를 공유해야 함을 명시
  - `life_cycle_lite_release_manifest.json`를 reviewer entrypoint로 추가해 산출물 경로/sha/version/render identity를 한 번에 재검수 가능하게 고정
...
- **LOCK 항목**은 v1.2.31 내에서 변경 금지입니다 (필요 시 후속 버전으로 버전업).
...
- v1.2.31의 유일한 P0 productization 대상은 `life_cycle-lite`입니다.
- 이 문서는 **backend P0 실행 계약**입니다. repo-wide rollout을 선언하려면 `frontend/app/page.tsx`, `frontend/lib/api.ts`, `frontend/app/chart/ChartClient.tsx`, 관련 E2E가 새 계약을 실제로 소비하도록 구현되어 있어야 하며, migration 단락/계획만으로는 부족합니다.
- backend P0라도 product-aware finalizer/front isolation, polished cache namespace, /pdf direct-call contract alignment은 같은 단계에서 함께 닫혀 있어야 합니다.
- `yearly_forecast`, `compatibility`는 스펙 유지 대상이지만 v1.2.31 출고 블로커는 아닙니다.
...
| `contract_version` | 문자열 | 적용 PRD 계약 버전 (`v1.2.31`) |
...
1. **인생 주기 리포트 (Life Cycle Map)**: 마하다샤 기반 인생 전체 주기 지도 + 현재 위치 + 방법론/valid_until UX + 단계별 확장 구조 *(v1.2.31 구현 단계: P0는 `life_cycle-lite`; 고점/저점 지도, 반복 패턴, 다음 3년 구체화는 P1에서 복원)*
2. **신년운세 리포트**: 연간 구간 + 월별 12슬롯 + 중요구간 2개 + 분야별 모듈
3. **궁합 리포트**: Ashtakoota 36점 기반 궁합 평가 + 조율 행동

> **v1.2.31 구현 단계 계약 (LOCK)**:
> - `life_cycle`는 유일한 P0 productization 대상입니다.
> - `yearly_forecast`, `compatibility`는 bugfix-only / regression-only 정책을 적용합니다.
> - P0 CTA는 미출시 상품명 대신 업데이트 알림/리마인드형 CTA-lite만 허용합니다.
...
> - 아래 상세 계약은 보존되며, v1.2.31에서는 P1 기능 미구현만으로 FAIL 처리하지 않습니다.

#### 7.1.1 목적/핵심 가치

- **핵심 질문**: "내 인생의 큰 파도는 언제 오고, 언제 내려가는가?"
- **엔진 기반**: Vimshottari Dasha 120년 주기 전체 계산값 활용
- **구조 전략 (LOCK)**: `life_cycle-full` 목표 구조는 인생 전체 70% + 다음 3년 구체화 30%를 유지한다. 단, v1.2.31의 `life_cycle-lite` P0는 `7.1.2`의 최소 출력 구조만 적용하며, 다음 3년 구체화는 P1에서만 복원한다.
...
> **v1.2.31 P0 필수 섹션 집합 (exact H2, order LOCK)**:
> - `cover/meta`
> - `How to use 1p`
> - `인생 구조 한 장 요약`
> - `4단계 인생 구조`
...
#### 7.1.5 고점/저점 지도 산출 기준 (결정론, LOCK, v1.2.31 구현 단계: P1)
...
#### 7.1.6 반복 패턴 분석 기준 (결정론, LOCK, v1.2.31 구현 단계: P1)
...
#### 7.1.7 다음 3년 구체화 섹션 (재구매 훅, LOCK, v1.2.31 구현 단계: P1)
...
- **업셀 트리거 (결정론)**: *(v1.2.31 P0에서는 CTA-lite만 필수, 최적화는 P1)*
...
### 10.0 v1.2.31 구현 단계 계약 (LOCK)

- 표준 테스트 실행 명령은 `python -m pytest`입니다.
- `pytest` CLI PATH 의존은 허용하지 않습니다.
- clean environment에서 `python -m pytest`가 재현되지 않으면 최종 출고 판정을 닫을 수 없습니다.
- stale test가 현재 코드 계약과 충돌하면, 구현보다 먼저 테스트 계약을 현재 런타임 기준으로 정렬해야 합니다.
- v1.2.31 P0 수용 기준은 `life_cycle-lite` 최소 구조 중심으로 판정합니다.
- 7.1.5 / 7.1.6 / 7.1.7의 상세 테스트는 계약 보존 대상이지만, v1.2.31 P0 미구현만으로 FAIL 처리하지 않습니다.
- `yearly_forecast` / `compatibility`는 bugfix-only 정책 위반 여부만 확인합니다.
- HF 16개 회귀 없음의 **최종 출고 판정**은 `cheap_validation_gate.py`의 `life_cycle-lite` release mode 또는 동일 로직을 호출하는 release gate runner 기준으로만 닫습니다.
...
- `CTA-lite` 하단에 동의 버튼/secondary control이 별도 존재 → `action_steps_contract_ok=False`  **(v1.2.31 P0)**
- generic front-contract/12챕터 scan 결과와 충돌하더라도, 최종 판정은 반드시 `life_cycle-lite` release mode 전용 helper 출력 기준으로 닫음

**인생 주기 리포트**:
- `assign_life_stages()`: 다샤 9개 → [2,2,2,3] / 4개 → [1,1,1,1] / 3개 → 3 stages / 1개 → 1 stage / 0개 → 빈 리스트  **(v1.2.31 P0 필수)**
...
- `compute_life_highs_lows()`: 9개 → 상위3/하위3/전환점5 / 5개 → 전환점 4개  **(v1.2.31 P1)**
...
- `compute_repeat_patterns()`: 동일 도메인 2회+ → 패턴 인식 / 1회 → 제외  **(v1.2.31 P1)**
...
- 다음 3년 구체화: 3개 슬롯 → 3개 출력 (패딩 없음) / 0개 → 고정 문구만 출력  **(v1.2.31 P1)**
...
> **운영 근거 (v1.2.31)**:
> - golden snapshot은 운영하지 않지만, v1.2.31는 P0 범위를 `life_cycle-lite`로 축소한 상태다.
...
> - 릴리즈 증적은 `PRD/release_evidence/v1_2_31/life_cycle_lite_manual_qa.md`, `PRD/release_evidence/v1_2_31/life_cycle_lite_sample_response.json`, `PRD/release_evidence/v1_2_31/life_cycle_lite_gate_summary.json`, `PRD/release_evidence/v1_2_31/life_cycle_lite_release_manifest.json` 4종으로 고정합니다.
...
> - `life_cycle_lite_sample_response.json`의 `meta.contract_version`, `life_cycle_lite_gate_summary.json`의 `contract_version`, `life_cycle_lite_release_manifest.json`의 `contract_version` / `release_evidence_dir` / `render_profile` / `request_fingerprint` / `evidence_case_id`는 모두 현재 정본 버전 `v1.2.31` 및 수동 QA 문서와 일치해야 합니다.
...
- **인생 주기 리포트 전용**:
  ...
  - v1.2.31 P0: 4단계 구조가 과하게 복잡하지 않고 한 번에 이해되는지 확인
  - v1.2.31 P0: P1 섹션 미노출이 오히려 문서를 더 명료하게 만드는지 확인
...
| v1.2.7 | P0 섹션 집합 정렬 / HF release gate 경로 고정 / valid_until_fallback meta 정렬 / CTA-lite 현실화 |
| v1.2.8 | HF2/HF3 release mode semantics 고정 / stage edge case 렌더 계약 고정 / P0 QA 범위 재정렬 |
...
| **v1.2.31 (현재, 진행 중)** | **pytest 블로커 해소 + `life_cycle-lite` P0 productization + release evidence traceability hardening** |
| v1.2.x | `v1.2.31` 수용 기준 전부 충족 후 `life_cycle-full` 착수 / yearly_forecast·compatibility 정리 |
...
1. clean-environment `python -m pytest` 재현성 확보
2. stale test (`backend/test_llm_token_limits.py` 등) 정렬
...
5. `life_cycle-lite` 분기 단일화 + full meta contract 고정
6. 4단계 구조 렌더 로직 정리
...
12. `life_cycle-lite` product-layer 기본 `llm_max_tokens=7000` 고정
...
- generic `_finalize_ai_reading_result()` / `_build_polished_reading_surface()` / `prepend_front_modules()` 경로를 그대로 두면, `life_cycle-lite` renderer를 추가해도 generic front modules가 다시 붙을 수 있습니다.
...
- 대응: v1.2.31 P0는 backend-only로 닫되, 전체 저장소 적용을 주장하려면 `frontend/app/page.tsx`, `frontend/app/chart/ChartClient.tsx`, `frontend/lib/api.ts`, 관련 E2E가 새 계약을 실제로 소비하도록 구현되어 있어야 합니다. migration 계획/노트는 보조 문서일 뿐 완료 근거가 아닙니다.

### 14.3 남은 리스크 — 테스트 환경

- 현재 워크스페이스에서는 `python -m pytest --version`부터 `No module named pytest`로 실패하므로, 표준 테스트 명령이 설치 단계에서 막힙니다.
- `backend/requirements.txt`만으로는 clean environment에서 pytest 실행 재현이 보장되지 않습니다.
- 루트에 `pytest.ini`, `pyproject.toml`, `tox.ini`가 없어 temp/log/cache 수집 제외 규칙도 고정돼 있지 않습니다.
- `backend/test_llm_token_limits.py`처럼 현재 런타임 상수/시그니처와 불일치하는 stale test가 남아 있으면 표준 테스트 계약 자체가 오염됩니다.
- 대응: 기능 개발보다 pytest 설치/실행 문서, dev dependency 정리, stale test 정렬을 선행합니다.
...
- 대응: `cheap_validation_gate.py`에 `life_cycle-lite` release mode를 추가하고, 그 다음에만 `backend/QUALITY_GATES.md`와 `README.md`의 최종 출고 문구를 전환합니다.
...
3. `life_cycle-lite` 경로 동작
4. `life_cycle` 응답이 full P0 meta contract(`as_of_utc`, `as_of_local`, `timezone_offset`, `valid_until`, `valid_until_fallback`, `onboarding_goal`, `current_mahadasha_planet`, `next_mahadasha_date`, `product_type`, `contract_version`, `render_profile`)를 local date 직렬화 규칙과 함께 유지
5. generic finalizer/front 경로가 `life_cycle-lite` 표면에 다시 붙지 않고, route cache key와 polished narrative cache namespace가 product-aware contract로 분리됨
...
13. micro fixture 계약 테스트 통과
14. 수동 QA 2건 통과
15. `yearly_forecast` / `compatibility`는 bugfix-only 정책 위반 없음
16. `PRD/release_evidence/v1_2_31/` 아래에 `life_cycle_lite_manual_qa.md`, `life_cycle_lite_sample_response.json`, `life_cycle_lite_gate_summary.json`, `life_cycle_lite_release_manifest.json`가 모두 존재하고, 수동 QA 문서의 `contract_version`, `release_evidence_dir`, `render_profile`, `request_fingerprint`, `evidence_case_id`, `commit_sha`, `release_manifest_path`가 현재 정본 `v1.2.31` 기준과 일치함
17. `life_cycle_lite_release_manifest.json`만 읽어도 4개 산출물의 경로, SHA-256, `contract_version`, `release_evidence_dir`, `render_profile`, `request_fingerprint`, `evidence_case_id`, `commit_sha`를 재검수할 수 있음
18. `backend/API.md`가 `/ai_reading`의 optional `product_type`, `life_cycle` 요청 입력, full P0 meta contract, bugfix-only 범위, backend-only P0 한계를 현재 버전 기준으로 설명함
19. `backend/QUALITY_GATES.md`와 `README.md`가 backend-only P0와 repo-wide rollout을 명확히 구분하고, 최종 release gate source of truth 문구가 현재 계약과 동일함
