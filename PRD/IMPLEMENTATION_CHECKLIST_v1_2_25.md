# IMPLEMENTATION_CHECKLIST v1.2.25 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_25.md`
- 점검 기준일: 2026-03-07
...
- Current state: `python -m pytest -q`는 temp/log/cache 디렉터리 수집 단계에서 막혀 전체 acceptance command로 쓰기 어렵습니다.
...
- Target state: v1.2.25 P0는 backend-only `life_cycle-lite` productization 계약을 잠그는 문서이며, repo-wide 완료 주장은 README/frontend migration까지 반영된 뒤에만 가능합니다.

---

## 0.6 필수 리스크 4개 (2026-03-07 latest)

1. `python -m pytest -q`가 최신 로컬 실행 기준 `161`개 `PermissionError`로 수집 단계에서 중단되며, `backend/requirements.txt`에는 `pytest`가 없고 `backend/test_llm_token_limits.py`도 현재 런타임과 충돌합니다.
2. 런타임 `/ai_reading` API와 cache key는 아직 target contract가 아니며, `product_type` / `life_cycle` 요청 스키마와 cache isolation이 미구현입니다.
3. frontend는 아직 `life_cycle-lite`를 노출하거나 소비할 수 없으므로, backend-only 실험이 아니라 repo-wide 적용 기준이면 blocker입니다.
4. 출고 gate source of truth가 아직 옛 기준이므로, `life_cycle-lite` 계약을 검증하지 못한 채 구 게이트만 통과하고 출고될 위험이 있습니다.
...
| `pytest` 실행 환경 | `BLOCKED` | 최신 로컬 재현 기준 collection 단계 `161`개 에러, clean env 재현성과 stale test 정렬 전까지 release blocker |
| frontend/client migration note | `PARTIAL` | 체크리스트와 API.md에는 반영됨, frontend 구현/README 반영 전까지 repo-wide 적용 blocker |
| micro fixture 체계 | `TODO` | generic fixture 1개만 있음 |
| token budget v1.2.25 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
`AUD-15` `TODO` `commercial_gate_helpers.py` 부재
...
`AUD-25` `BLOCKED` `python -m pytest` clean-env / collection contract broken (release blocker)
    - 근거: 현재 환경에서는 `python -m pytest --version`이 성공 (`pytest 9.0.2`)
    - 추가 근거: `backend/requirements.txt`에 `pytest` 없음
    - 추가 근거: 루트에 `pytest.ini`, `pyproject.toml`, `tox.ini`가 없어 수집 제외 규칙이 고정돼 있지 않음
    - 현재 재현: `python -m pytest -q` 기준 `logs/tmp*`, `pytest-cache-files-*`, `tmp*` 수집 단계에서 `161`개 `PermissionError`로 중단
    - 의미: 현재 로컬 인터프리터에 우연히 설치돼 있을 뿐 아니라, 현 워크스페이스에서는 기본 수집 계약 자체도 깨져 있음
    - release 기준: 이 항목이 닫히기 전에는 v1.2.25 P0 출고를 닫지 않음
    - 해야 할 일:
      - pytest 수집 제외 규칙부터 고정 (`logs/`, temp/cache 계열)
      - dev requirements 또는 requirements 분리
      - 최소한 로컬/CI에서 문서화된 명령 1개로 `python -m pytest` 가능하게 만들 것

`AUD-26` `BLOCKED` stale test 정렬 필요: `backend/test_llm_token_limits.py`
    ...
    - 현재 재현: `python -m pytest backend/test_llm_token_limits.py -q` 기준 3개 실패
    - 해야 할 일:
      - 이 테스트를 현재 아키텍처에 맞게 정리
      - v1.2.25용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.25에서 직접 변경 대상이 아님

PRD v1.2.25 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
1. `BLOCKED` `python -m pytest` clean-env 재현성 확보
   - 현재 상태: 현재 인터프리터에서는 `python -m pytest --version` 성공
   - Done 기준: clean environment에서도 같은 명령이 재현 가능
...
3. `BLOCKED` stale test 정리: `backend/test_llm_token_limits.py`
   - 현재 문제:
     - 상수 기대값이 현재 코드와 불일치
...
25. `BLOCKED` `python -m pytest` clean-env / collection contract broken (release blocker)
    - source of truth: `Phase A` item 1 참조
    - 의미: renderer/contract/gate 작업을 진행하더라도 이 blocker가 닫히기 전에는 P0 출고를 닫지 않음
...
37. `PARTIAL` `backend/API.md`에 `/ai_reading` `product_type` / `life_cycle` 공개 계약 추가
    - Done 기준: `backend/API.md`가 `/ai_reading`의 optional `product_type`, `life_cycle` 요청 입력, full P0 meta contract, bugfix-only 범위를 설명
    - 현재 상태: 공개 API 목표 계약과 GET list CSV 직렬화 규칙까지 문서화됨, 남은 쟁점은 런타임 API / cache 정합성
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_25.md` 정본과 코드 정합 재검수
41. `TODO` `README.md` 범위/출고 기준 정리
...
42. `TODO` frontend/client/API/cache 구현 + E2E 반영 (repo-wide 후속)
...
1. 테스트 환경 재현성 복구 (`python -m pytest` clean-env 기준) (`Phase A` #1~#4)
2. stale test 정리 (`test_llm_token_limits.py` 우선) (`Phase A` #3)
3. `product_type` + `life_cycle` 요청 스키마 + orchestrator 얇게 추가 (`Phase B` #6~#9)
4. `render_contract.py`로 full meta contract 고정 (`Phase B` #10)
5. `commercial_quality_constants.py` shared constants + `life_cycle_helpers.py` 추가 (`Phase C` #11~#20)
6. `life_cycle-lite` payload builder 구현 (`Phase C` #12~#17)
7. `life_cycle_lite_renderer.py` 구현 (`Phase D` #19~#24)
8. response meta local-date 직렬화 (`Phase F` #34)
9. contract test 4종 + `cheap_validation_gate` release mode 추가 (`Phase E` #25~#32)
10. `backend/QUALITY_GATES.md` / `README.md` release gate source 전환 (`Phase G` #38~#41)
11. fallback/edge 수동 QA 2건 수행 (`Phase E` #30~#31)
12. repo-wide 적용이 필요하면 frontend/client note만이 아니라 `frontend/app/page.tsx`, `frontend/lib/api.ts`, `frontend/app/chart/ChartClient.tsx` 구현까지 반영 (`Phase G` #42)
13. PDF 경로 점검 (`Phase F` #35~#36)
14. 그 다음에만 개선 작업
...
아래가 모두 만족되면 v1.2.25 기준 P0 완료로 봅니다.

1. `python -m pytest`가 clean environment에서도 재현 가능
2. stale test(`backend/test_llm_token_limits.py` 등)가 현재 코드 계약과 정렬됨
...
6. `life_cycle-lite` markdown이 `cover/meta` 포함 exact H2 order로 deterministic 하게 생성됨
7. `cheap_validation_gate.py`의 `life_cycle-lite` release mode 기준 HF 16개 회귀 없음
8. `backend/QUALITY_GATES.md`와 `README.md`의 release gate 문구가 동일 source of truth로 전환됨
9. 정상 fixture에서는 4단계 + current stage 1개, edge fixture에서는 0~3단계 허용 및 0단계면 current stage 없음
10. `next_mahadasha_date` 결측/과거 시 fallback UX가 자연스럽게 동작함
11. 단일 LLM 호출 정책 유지
12. `life_cycle` 상품 경로에서 product-layer 기본값 `7000`, soft QA range `6000–8000`, hard cap `9000`이 고정됨
13. P1 기능이 출력에 섞이지 않음
14. `backend/API.md`가 `/ai_reading`의 `product_type`, `life_cycle` 요청 입력, 응답 meta contract를 현재 기준으로 설명함
15. README/운영 문서가 backend P0와 repo-wide rollout을 구분함
16. micro fixture 2건 + 수동 QA 2건 통과
