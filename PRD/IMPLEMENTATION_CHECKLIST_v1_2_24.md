# IMPLEMENTATION_CHECKLIST v1.2.24 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_24.md`
- 점검 기준일: 2026-03-06
...
- Target state: v1.2.24 P0는 backend-only `life_cycle-lite` productization 계약을 잠그는 문서이며, repo-wide 완료 주장은 README/frontend migration까지 반영된 뒤에만 가능합니다.

---

## 0.6 필수 리스크 4개 (2026-03-06 latest)
...
| token budget v1.2.24 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
19. `TODO` `PLANET_LABEL_MAP` / consumer label mapping shared constants 없음
...
20. `TODO` `LIFE_STAGE_LABELS` / `TRANSITION_INTENSITY_THRESHOLDS` shared constants 없음
...
22. `TODO` full P0 meta contract가 실제 backend 응답에 없음
...
25. `BLOCKED` `python -m pytest` clean-env / collection contract broken (release blocker)
    ...
    - release 기준: 이 항목이 닫히기 전에는 v1.2.24 P0 출고를 닫지 않음
    - 해야 할 일:
...
26. `BLOCKED` stale test 정렬 필요: `backend/test_llm_token_limits.py`
    ...
    - 해야 할 일:
      ...
      - v1.2.24용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.24에서 직접 변경 대상이 아님

PRD v1.2.24 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_24.md` 정본과 코드 정합 재검수
...
1. 테스트 환경 재현성 복구 (`python -m pytest` clean-env 기준)
2. stale test 정리 (`test_llm_token_limits.py` 우선)
3. `product_type` + `life_cycle` 요청 스키마 + orchestrator 얇게 추가
4. `render_contract.py`로 full meta contract 고정
5. `commercial_quality_constants.py` shared constants + `life_cycle_helpers.py` 추가
6. `life_cycle-lite` payload builder 구현
7. `life_cycle_lite_renderer.py` 구현
8. response meta local-date 직렬화
9. contract test 4종 + `cheap_validation_gate` release mode 추가
10. `backend/QUALITY_GATES.md` / `README.md` release gate source 전환
11. fallback/edge 수동 QA 2건 수행
12. repo-wide 적용이 필요하면 frontend/client note만이 아니라 `frontend/app/page.tsx`, `frontend/lib/api.ts`, `frontend/app/chart/ChartClient.tsx` 구현까지 반영
13. PDF 경로 점검
14. 그 다음에만 개선 작업
...
아래가 모두 만족되면 v1.2.24 기준 P0 완료로 봅니다.
