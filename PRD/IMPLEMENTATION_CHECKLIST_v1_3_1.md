# IMPLEMENTATION_CHECKLIST v1.3.1 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_3_1.md`
- 점검 기준일: 2026-03-11
...
- Target state: v1.3.1 P0는 backend-only `life_cycle-lite` productization 계약을 잠그는 문서이며, repo-wide 완료 주장은 README/frontend migration까지 반영된 뒤에만 가능합니다. 단, backend-only라도 product-aware finalizer/cache/PDF contract는 함께 닫혀 있어야 합니다.
...
| token budget v1.3.1 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
`AUD-25` `PARTIAL` `python -m pytest` baseline 실행 가능하나 backend suite는 아직 clean green 아님
    ...
    - release 기준: test runner 부재 이슈는 닫혔지만, stale cluster / temp write permission cluster가 남아 있으면 v1.3.1 P0 sign-off를 닫지 않음
    - 해야 할 일:
...
`AUD-26` `BLOCKED` stale contract test cluster 정렬 필요
    ...
    - 해야 할 일:
      - 각 테스트를 `현재 코드가 맞는지` / `테스트 기대가 stale한지`로 재분류
...
- 이 값은 v1.3.1에서 직접 변경 대상이 아님

PRD v1.3.1 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
3. `BLOCKED` stale contract test cluster 정리
   - 현재 문제:
   ...
   - Done 기준: 각 테스트가 현재 구현 또는 product-layer 정책에 맞게 재작성 또는 폐기 근거와 함께 정리됨
...
31. `TODO` 수동 QA 템플릿 작성
    - 권장 경로: `PRD/release_evidence/v1_3_1/life_cycle_lite_manual_qa.md`
    ...
    - 필수 필드:
      ...
      - reviewer
      - run date (Asia/Seoul)
    - Done 기준: 2건 모두 채워져 있고, 문서 상단 메타만 읽어도 현재 정본 `v1.3.1` 기준의 release candidate와 연결된 manifest 경로를 확인할 수 있음

31-a. `TODO` release gate summary 저장
    - 권장 경로: `PRD/release_evidence/v1_3_1/life_cycle_lite_gate_summary.json`
    ...
    - Done 기준: 자동 gate가 실제 release source of truth로 통과했는지 별도 로그 검색 없이 확인 가능하고, `contract_version == v1.3.1`이며 `render_profile` / `request_fingerprint` / `evidence_case_id`가 수동 QA 문서 및 manifest와 일치함

31-b. `TODO` 샘플 응답 1개 저장
    - 권장 경로: `PRD/release_evidence/v1_3_1/life_cycle_lite_sample_response.json`
    ...
    - Done 기준: 문서/QA/release gate 검토자가 코드 실행 없이도 계약 필드와 최종 렌더 구조를 확인할 수 있고, `meta.contract_version == v1.3.1`이며 `meta.render_profile`이 gate summary 및 manifest와 일치함

31-c. `TODO` release evidence manifest 저장
    - 권장 경로: `PRD/release_evidence/v1_3_1/life_cycle_lite_release_manifest.json`
    ...
    - 필수 포함:
      ...
      - `gate_summary_sha256`
    - Done 기준: reviewer가 manifest 하나만 읽어도 세 증적 파일의 경로/해시/버전/render/request identity를 현재 정본 `v1.3.1` 기준으로 재검수할 수 있음
...
39. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_3_1.md` 정본과 코드 정합 재검수
...
1. backend pytest baseline 정리 (`stale contract cluster` + `temp write permission cluster`) (`Phase A` #1~#4)
2. `product_type` + `life_cycle` 요청 스키마 + orchestrator 얇게 추가 (`Phase B` #6~#9)
3. `render_contract.py`로 full meta contract 고정 (`Phase B` #10)
4. product-aware finalize branch + generic front 재부착 차단 + polished cache namespace 분리 (`Phase B` + `Phase F`)
...
8. response meta local-date 직렬화 + `/pdf` direct call contract 정렬 (`Phase F` #34~#36)
9. contract test 4종 (`#27`, `#28`, `#28-a`, `#30`) + `cheap_validation_gate` release mode 추가 (`Phase E` `#25~#32`)
10. `backend/API.md` + `backend/QUALITY_GATES.md` + `README.md` 문서 계약 동기화 (`Phase G` #37~#40)
11. 수동 QA 2건 + `release_evidence` 4종(`manual_qa`, `sample_response`, `gate_summary`, `release_manifest`) 저장 (`Phase E` `#30~#31-c`)
12. repo-wide 적용이 필요하면 frontend/client note만이 아니라 `frontend/app/page.tsx`, `frontend/lib/api.ts`, `frontend/app/chart/ChartClient.tsx` 구현까지 반영 (`Phase G` #41)
13. 그 다음에만 개선 작업
...
아래가 모두 만족되면 v1.3.1 기준 P0 완료로 봅니다.

1. `python -m pytest` 표준 명령이 실행 가능하고, backend baseline failure inventory가 문서/CI 기준으로 고정됨
2. stale contract test cluster와 temp write permission cluster가 현재 코드 계약과 정렬됨
...
19. `PRD/release_evidence/v1_3_1/` 아래에 `life_cycle_lite_manual_qa.md`, `life_cycle_lite_sample_response.json`, `life_cycle_lite_gate_summary.json`, `life_cycle_lite_release_manifest.json`가 모두 존재하고, manifest를 포함한 4종이 같은 `contract_version`, `render_profile`, `request_fingerprint`, `evidence_case_id`, `commit_sha` 기준으로 현재 정본 `v1.3.1`과 일치함
