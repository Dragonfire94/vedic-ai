# CHANGELOG_PRD v1.3.1

- Source PRD: `PRODUCT_SPEC_PRD_v1_3_1.md`
- Last updated (Asia/Seoul): 2026-03-11

## 변경 로그

### v1.3.1 *(현재)*
- `v1_2_31` 정본의 상세 계약과 섹션 구조를 유지한 채, 현재 저장소 상태를 반영하는 버전업 문서로 재작성
- `python -m pytest` 미설치 전제를 제거하고, 실제 baseline(`367 passed, 14 failed, 1 skipped`, `-p no:cacheprovider`)을 기준으로 테스트 리스크를 재분류
- `backend/test_llm_token_limits.py`는 현재 런타임 계약 기준으로 정렬되었음을 반영하고, 남은 실패를 stale contract cluster / temp write permission cluster로 나눠 기록
- `release_evidence` 경로, `contract_version`, 수용 기준, 추천 구현 순서를 `v1.3.1` 기준으로 갱신
### v1.3.1 *(현재)*
