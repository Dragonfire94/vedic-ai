# PRODUCT_SPEC_PRD v1.2.23 — 상업용 베딕 리포트 엔진 (Contract-Complete)

- **Status:** LOCKED (상세 계약 보존 + v1.2.23 실행 범위 분리)
...
### v1.2.23 *(현재)*
- backend P0 실행 계약과 repo-wide rollout 해석을 분리하고, frontend/client migration 누락 시 "전체 저장소 적용 완료"로 간주하지 않도록 명시
- `python -m pytest` clean-environment 재현성과 stale test 정렬을 출고 블로커로 승격
- `backend/QUALITY_GATES.md` / `README.md`의 release gate source of truth를 `cheap_validation_gate.py` `life_cycle-lite` release mode로 전환해야만 최종 출고를 닫을 수 있도록 명시
- `life_cycle-lite` P0 메타 계약을 full field set으로 확장하고, `render_contract`/contract test 기준을 같은 범위로 정렬
- `life_cycle-lite` 요청 개인화 계약(`subject_name`, `onboarding_goal`, `focus_tokens`, `concern_tokens`, `occupation_context`, `relationship_status`)을 추가하고 fallback semantics를 잠금
- `cover/meta` H2를 P0 exact order와 release gate 양쪽에 동일하게 포함하고, HF11 narrative 시작 섹션 계산에서는 제외하도록 정렬
- `valid_until` / `next_mahadasha_date` 소비자 노출 포맷을 local date 직렬화(`YYYY-MM-DD`)로 통일
- `PLANET_LABEL_MAP`, `VALID_PLANET_CODES`, `PLANET_DOMAIN_MAP`, `LIFE_STAGE_LABELS`, `TRANSITION_INTENSITY_THRESHOLDS`의 소유 위치를 `commercial_quality_constants.py`로 고정
- `backend/API.md` target public contract는 반영되었고, checklist 근거 문구는 runtime code excluding docs/tests 기준으로 더 엄밀하게 좁힘
- `python -m pytest -q`가 temp/log/cache 디렉터리 수집 단계에서 `149`개 `PermissionError`로 중단되는 현 상태를 release blocker로 명시

## Current State vs Target State (2026-03-06)

- Current state: runtime backend는 아직 generic `/ai_reading` 경로와 generic cache key를 사용합니다.
- Current state: frontend 진입점과 E2E는 아직 `/chart` + BTR + legacy `AIReadingResponse` 계약을 가정합니다.
- Current state: 운영 문서의 release gate source of truth는 아직 `golden_sample_runner` / `fast_llm_gate` 기준입니다.
...
- **LOCK 항목**은 v1.2.23 내에서 변경 금지입니다 (필요 시 후속 버전으로 버전업).
...
- v1.2.23의 유일한 P0 productization 대상은 `life_cycle-lite`입니다.
- 이 문서는 **backend P0 실행 계약**입니다. repo-wide rollout을 선언하려면 frontend/client/API/cache migration 단락 또는 동등한 구현 상태가 별도로 존재해야 합니다.
- `yearly_forecast`, `compatibility`는 스펙 유지 대상이지만 v1.2.23 출고 블로커는 아닙니다.
...
| `contract_version` | 문자열 | 적용 PRD 계약 버전 (`v1.2.23`) |
...
1. **인생 주기 리포트 (Life Cycle Map)**: 마하다샤 기반 인생 전체 주기 지도 + 현재 위치 + 방법론/valid_until UX + 단계별 확장 구조 *(v1.2.23 구현 단계: P0는 `life_cycle-lite`; 고점/저점 지도, 반복 패턴, 다음 3년 구체화는 P1에서 복원)*
2. **신년운세 리포트**: 연간 구간 + 월별 12슬롯 + 중요구간 2개 + 분야별 모듈
3. **궁합 리포트**: Ashtakoota 36점 기반 궁합 평가 + 조율 행동

> **v1.2.23 구현 단계 계약 (LOCK)**:
...
> - 아래 상세 계약은 보존되며, v1.2.23에서는 P1 기능 미구현만으로 FAIL 처리하지 않습니다.
...
| # | 섹션 | 엔진 입력 | 분량 캡 |
|---|---|---|---|
| 0 | cover/meta | 이름, 출생정보, 적용 기간(출생~80세) | 1페이지 |
| 1 | How to use 1p | 체크리스트 + 복구 플랜 | 1페이지 |
| 2 | 인생 구조 한 장 요약 | 4단계 구조 요약 + 현재 위치 훅 | 1페이지 |
| 3 | 4단계 인생 구조 | stage 0~4 출력 계약 | 1페이지 |
| 4 | 현재 위치 | current stage + 다음 전환 힌트 | 4~6줄 |
| 5 | 마하다샤 단계 목록 | 전체 다샤 날짜 + 주제 라벨 | 행당 4~6줄, 총 9행 이내 |
| 6 | 방법론 카드 | 6줄 고정 카드 | HF15 자동 검증 |
| 7 | valid_until 설명 | 갱신 시점 + 리마인드 행동 | 4~6줄 |
| 8 | CTA-lite | 리마인드형 단일 행동 | 1줄 + 버튼 1개 |
| 9 | 면책/윤리/데이터 보호 | 필수 5줄 | 1페이지 이내 |
| 10 | 인생 고점/저점 지도 | pressure_score 기반 | 상승 3 + 전환점 5 + 경계 3 |
| 11 | 반복 패턴 분석 | 행성 도메인별 반복 구간 | 3개 영역 × 최대 4줄 |
| 12 | 다음 3년 구체화 | 현재 부크티 전환 일정 | 최대 5슬롯 |

> **v1.2.23 P0 필수 섹션 집합 (exact H2, order LOCK)**:
> - `cover/meta`
> - `How to use 1p`
> - `인생 구조 한 장 요약`
> - `4단계 인생 구조`
...
#### 7.1.5 고점/저점 지도 산출 기준 (결정론, LOCK, v1.2.23 구현 단계: P1)
...
#### 7.1.6 반복 패턴 분석 기준 (결정론, LOCK, v1.2.23 구현 단계: P1)
...
#### 7.1.7 다음 3년 구체화 섹션 (재구매 훅, LOCK, v1.2.23 구현 단계: P1)
...
- **업셀 트리거 (결정론)**: *(v1.2.23 P0에서는 CTA-lite만 필수, 최적화는 P1)*
...
- v1.2.23 P0 수용 기준은 `life_cycle-lite` 최소 구조 중심으로 판정합니다.
- 7.1.5 / 7.1.6 / 7.1.7의 상세 테스트는 계약 보존 대상이지만, v1.2.23 P0 미구현만으로 FAIL 처리하지 않습니다.
...
- `CTA-lite` 하단에 동의 버튼/secondary control이 별도 존재 → `action_steps_contract_ok=False`  **(v1.2.23 P0)**
- generic front-contract/12챕터 scan 결과와 충돌하더라도, 최종 판정은 반드시 `life_cycle-lite` release mode 전용 helper 출력 기준으로 닫음

**인생 주기 리포트**:
- `assign_life_stages()`: 다샤 9개 → [2,2,2,3] / 4개 → [1,1,1,1] / 3개 → 3 stages / 1개 → 1 stage / 0개 → 빈 리스트  **(v1.2.23 P0 필수)**
...
- `compute_life_highs_lows()`: 9개 → 상위3/하위3/전환점5 / 5개 → 전환점 4개  **(v1.2.23 P1)**
...
- `compute_repeat_patterns()`: 동일 도메인 2회+ → 패턴 인식 / 1회 → 제외  **(v1.2.23 P1)**
...
- 다음 3년 구체화: 3개 슬롯 → 3개 출력 (패딩 없음) / 0개 → 고정 문구만 출력  **(v1.2.23 P1)**
...
> **운영 근거 (v1.2.23)**:
> - golden snapshot은 운영하지 않지만, v1.2.23는 P0 범위를 `life_cycle-lite`로 축소한 상태다.
...
- **인생 주기 리포트 전용**:
  ...
  - v1.2.23 P0: 4단계 구조가 과하게 복잡하지 않고 한 번에 이해되는지 확인
  - v1.2.23 P0: P1 섹션 미노출이 오히려 문서를 더 명료하게 만드는지 확인
...
| **v1.2.23 (현재)** | **full meta contract 정렬 / personalization request contract 추가 / cover-meta gate 정렬 / valid_until local-date 직렬화 통일** |
| v1.2.x | `v1.2.23` 수용 기준 전부 충족 후 `life_cycle-full` 착수 / yearly_forecast·compatibility 정리 |
...
3. `life_cycle-lite` 분기 단일화
4. 4단계 구조 렌더 로직 정리
5. valid_until fallback UX 유지
6. 방법론 카드 및 CTA-lite 고정 문구 정리
7. micro fixture 기반 계약 테스트 + `cheap_validation_gate.py` life_cycle-lite release mode 추가
8. `backend/QUALITY_GATES.md` / `README.md` release gate source of truth 전환
9. `life_cycle-lite` product-layer 기본 `llm_max_tokens=7000` 고정
10. `yearly_forecast` / `compatibility` bugfix-only 상태 고정
...
### 14.2-a 남은 리스크 — repo-wide 적용 해석

- 현재 홈 진입점은 정확한 출생 시각만 `/chart` 경로로 보내고, 대략/모름은 BTR 쪽으로 보냅니다. 또한 frontend client는 여전히 legacy `AIReadingResponse`와 generic `/ai_reading` 계약을 가정합니다.
- 대응: v1.2.23 P0는 backend-only로 닫되, 전체 저장소 적용을 주장하려면 `frontend/app/page.tsx`, `frontend/app/chart/ChartClient.tsx`, `frontend/lib/api.ts` 기준 migration 계획 또는 구현 상태를 함께 갱신합니다.

### 14.3 남은 리스크 — 테스트 환경

- 현재 인터프리터에서는 `python -m pytest`가 동작하더라도, `backend/requirements.txt`만으로는 clean environment 재현이 보장되지 않을 수 있음.
- 현재 워크스페이스에서 `python -m pytest -q`는 `logs/tmp*`, `pytest-cache-files-*`, `tmp*` 수집 단계에서 `149`개 `PermissionError`로 중단됩니다.
- 루트에 `pytest.ini`, `pyproject.toml`, `tox.ini`가 없어 temp/log/cache 수집 제외 규칙이 고정돼 있지 않습니다.
- `backend/test_llm_token_limits.py`처럼 현재 런타임 상수/시그니처와 불일치하는 stale test가 남아 있으면 표준 테스트 계약 자체가 오염됩니다.
- 대응: 기능 개발보다 pytest 수집 설정, dev dependency/설치 문서 정리, stale test 정렬을 선행.
...
- 대응: v1.2.23에서는 `life_cycle-lite`까지만 P0로 닫고, full 기능은 P1로 분리.
...
5. 정상 fixture에서는 4단계 + current stage 1개, edge fixture에서는 0~3단계 허용 및 0단계면 current stage 없음
6. `cheap_validation_gate.py`의 `life_cycle-lite` release mode 기준 HF 16개 회귀 없음
7. `backend/QUALITY_GATES.md`와 `README.md`의 release gate 문구가 동일 source of truth로 전환됨
8. valid_until fallback UX 유지
9. 리포트당 LLM 호출 1회 초과 없음
10. `life_cycle` 경로의 product-layer 기본값 `7000`, soft QA 범위 `6000–8000`, hard cap `9000`이 유지됨
11. micro fixture 계약 테스트 통과
12. 수동 QA 2건 통과
13. `yearly_forecast` / `compatibility`는 bugfix-only 정책 위반 없음
