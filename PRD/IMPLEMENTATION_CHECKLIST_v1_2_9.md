# IMPLEMENTATION_CHECKLIST v1.2.9 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_9.md`
- 점검 기준일: 2026-03-06
...
| token budget v1.2.9 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
26. `TODO` 현재 테스트 스위트 일부가 코드와 불일치할 가능성 높음
    ...
    - 해야 할 일:
      ...
      - v1.2.9용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.9에서 직접 변경 대상이 아님

PRD v1.2.9 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
27. `TODO` 최소 contract test 1: 메타 필드
    - 체크:
      ...
      - `contract_version == v1.2.9` 또는 구현 당시 최신값
...
29. `TODO` `cheap_validation_gate.py` life_cycle-lite release mode 추가
    - 목적: 최종 출고 게이트에서 HF2/HF3를 product-specific semantics로 판정
    - 구현 원칙: generic front-contract/12챕터 scanner를 그대로 재사용하지 말고, `life_cycle-lite` 전용 helper(`front_contract_ok`, `action_steps_contract_ok` 계산용)를 추가
    - `front_contract_ok = true` 조건:
      - required H2 `How to use 1p`, `인생 구조 한 장 요약`, `4단계 인생 구조`, `현재 위치`, `마하다샤 단계 목록`, `방법론 카드`, `valid_until 설명`, `면책/윤리/데이터 보호`, `CTA-lite`가 각 1회
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_9.md` 정본과 코드 정합 재검수
...
아래가 모두 만족되면 v1.2.9 기준 P0 완료로 봅니다.
...
6. 정상 fixture에서는 4단계 + current stage 1개, edge fixture에서는 0~3단계 허용 및 0단계면 current stage 없음
