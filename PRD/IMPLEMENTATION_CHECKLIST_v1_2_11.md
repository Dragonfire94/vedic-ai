# IMPLEMENTATION_CHECKLIST v1.2.11 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_11.md`
- 점검 기준일: 2026-03-06
...
| token budget v1.2.11 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
26. `TODO` 현재 테스트 스위트 일부가 코드와 불일치할 가능성 높음
    ...
    - 해야 할 일:
      ...
      - v1.2.11용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.11에서 직접 변경 대상이 아님

PRD v1.2.11 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
27. `TODO` 최소 contract test 1: 메타 필드
    - 체크:
      ...
      - `contract_version == v1.2.11` 또는 구현 당시 최신값
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_11.md` 정본과 코드 정합 재검수
...
아래가 모두 만족되면 v1.2.11 기준 P0 완료로 봅니다.
