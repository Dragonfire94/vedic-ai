# IMPLEMENTATION_CHECKLIST v1.2.8 (Current Project Audit)

- 기준 PRD: `PRODUCT_SPEC_PRD_v1_2_8.md`
- 점검 기준일: 2026-03-06
...
| token budget v1.2.8 정렬 | `PARTIAL` | PRD는 정렬되었지만 코드 product-layer 적용은 미구현 |
...
26. `TODO` 현재 테스트 스위트 일부가 코드와 불일치할 가능성 높음
    ...
    - 해야 할 일:
      ...
      - v1.2.8용 상품 토큰 정책은 global constant보다 product-layer에서 검사하는 방식으로 고정
...
- 이 값은 v1.2.8에서 직접 변경 대상이 아님

PRD v1.2.8 요구 (`life_cycle-lite` product-layer):
- default `7000`
...
13. `TODO` 현재 위치 계산
    ...
    - Done 기준: 정상 fixture에서는 current stage가 정확히 1개, 빈 stage fixture에서는 current stage가 없음
...
17. `TODO` 4단계 요약 데이터 구조 설계
    ...
    - Done 기준: renderer가 stage count 0~4를 추가 계산 없이 바로 출력 가능
...
20. `TODO` 섹션 구조 고정
    ...
    - 주의: stage가 1~3개면 존재하는 stage만 출력하고, 0개면 fallback summary만 출력하며 current line을 생략함
...
27. `TODO` 최소 contract test 1: 메타 필드
    - 체크:
      ...
      - `contract_version == v1.2.8` 또는 구현 당시 최신값
...
39. `TODO` 샘플 응답 1개 저장
40. `TODO` 구현 완료 후 `PRODUCT_SPEC_PRD_v1_2_8.md` 정본과 코드 정합 재검수
...
아래가 모두 만족되면 v1.2.8 기준 P0 완료로 봅니다.
