# CHANGELOG_PRD v1.3.2

- Source PRD: `PRODUCT_SPEC_PRD_v1_3_2.md`
- Last updated (Asia/Seoul): 2026-03-11

## 변경 로그

### v1.3.2 *(현재)*
- 체크리스트 안의 `pytest` 상태 충돌을 정리해, item 25를 `실행은 가능하지만 release blocker가 남아 있음`으로 통일
- `backend/API.md`, `backend/QUALITY_GATES.md`, `README.md`가 정렬되기 전까지 PRD v1.3.2를 interim runtime / release source of truth로 명시
- 현재 `367 passed, 14 failed, 1 skipped` baseline이 로컬 dirty worktree 기준임을 PRD/체크리스트에 명시하고, 구현 전에 baseline 정렬분을 먼저 커밋하라고 권고
- Phase A와 추천 구현 순서를 `runner 고정 + failure inventory 고정 + tempdir 분리` 우선으로 재배치하고, generic stale test 재작성은 product path 이후로 미룸
- `product_orchestrator.py`, `render_contract.py`, `life_cycle_helpers.py`, `commercial_gate_helpers.py`를 필수 파일 목록이 아니라 권장 구조로 완화하고, 완료조건을 behavior / test / gate 중심으로 재정렬
- `backend/API.md` 공개 계약과 `backend/QUALITY_GATES.md` / `README.md` release gate 문구는 아직 실제 운영 문서에 반영되지 않았음을 명시하고, 정렬 전 interim 배너를 유지
...
## Current State vs Target State (2026-03-11)
