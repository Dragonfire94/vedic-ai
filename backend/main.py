...
from backend.llm_service import refine_reading_with_llm, build_llm_structural_prompt, configure_llm_service
...
configure_llm_service(
    ...
    _validate_deterministic_llm_blocks=_validate_deterministic_llm_blocks,
    build_ai_psychological_input=build_ai_psychological_input,
    _get_atomic_chart_interpretations=_get_atomic_chart_interpretations,
    compute_chapter_blocks_hash=compute_chapter_blocks_hash,
    _candidate_openai_models=_candidate_openai_models,
    _build_openai_payload=_build_openai_payload,
    _emit_llm_audit_event=_emit_llm_audit_event,
    _normalize_long_paragraphs=_normalize_long_paragraphs,
    logger=logger,
)
...
# ------------------------------------------------------------------------------
# PDF helpers: custom Flowable components
# ------------------------------------------------------------------------------
SouthIndianChart = pdf_service.SouthIndianChart
create_pdf_styles = pdf_service.create_pdf_styles
load_pdf_layout_config = pdf_service.load_pdf_layout_config
...
async def generate_pdf(
    ...
    with BytesIO() as buffer:
        ...
        planet_table.setStyle(TableStyle([
            ('FONTNAME', (0, 0), (-1, 0), pdf_service.PDF_FONT_BOLD),
            ('FONTNAME', (0, 1), (-1, -1), pdf_service.PDF_FONT_REG),
            ('FONTSIZE', (0, 0), (-1, -1), 9),
        ...
        def _draw_page_chrome(canvas, _doc):
            ...
            canvas.setFont(pdf_service.PDF_FONT_BOLD, 8)
            canvas.setFillColor(header_color)
            canvas.drawString(_doc.leftMargin, A4[1] - _doc.topMargin + 14, "Vedic AI Report")
            canvas.setFont(pdf_service.PDF_FONT_REG, 8)
